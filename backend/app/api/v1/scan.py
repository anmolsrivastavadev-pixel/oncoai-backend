...
ANALYSIS_TIMEOUT = 90  # seconds — Cloudflare tunnel free tier has ~100s limit
MAX_IMAGE_DIM = 1024  # downscale large images to this max dimension before analysis
...
def analyze_color(image: np.ndarray, contour: np.ndarray) -> tuple:
    ...
    # Downsample large pixel sets to prevent k-means from hanging
    max_pixels = 10000
...
def _run_image_analysis_with_logging(img: np.ndarray, filename: str = "unknown") -> dict:
    """Wrapper that adds per-step timing logs for debugging slow analysis."""
    ...
    logger.info(f"SCAN [{filename}]: Step 1/5 quality_check: blur={blur_score} brightness={lighting['brightness']:.0f} — {(time.time() - step_t0)*1000:.0f}ms")

    step_t0 = time.time()
    contour, mask, found = segment_lesion(img)
    logger.info(f"SCAN [{filename}]: Step 2/5 segmentation: found={found} — {(time.time() - step_t0)*1000:.0f}ms")
    ...
    if found and contour is not None:
        ...
        logger.info(f"SCAN [{filename}]: Step 3/5 asymmetry: score={asym} — {(time.time() - step_t0)*1000:.0f}ms")

        step_t0 = time.time()
        border, border_detail, border_recs = analyze_border(img, contour)
        features.append(border_detail)
        warnings.extend(border_recs)
        logger.info(f"SCAN [{filename}]: Step 4a/5 border: score={border} — {(time.time() - step_t0)*1000:.0f}ms")

        step_t0 = time.time()
        color_var, color_detail, color_recs = analyze_color(img, contour)
        features.append(color_detail)
        warnings.extend(color_recs)
        logger.info(f"SCAN [{filename}]: Step 4b/5 color_kmeans: score={color_var} — {(time.time() - step_t0)*1000:.0f}ms")

        step_t0 = time.time()
        diam, diam_note = estimate_diameter(contour, img)
        features.append(diam_note)
        logger.info(f"SCAN [{filename}]: Step 5/5 diameter: {diam}mm — {(time.time() - step_t0)*1000:.0f}ms")
    else:
        logger.warning(f"SCAN [{filename}]: No lesion found — skipping feature extraction (segmentation took {(time.time() - pipeline_t0)*1000:.0f}ms)")
    ...
    logger.info(f"SCAN [{filename}]: Pipeline TOTAL: {pipeline_elapsed:.0f}ms")
...
# (_run_image_analysis_with_logging above is the canonical pipeline)


# ─── MAIN ENDPOINT ──────────────────────────────────────────

def _save_file_sync(content: bytes, file_path: str) -> None:
    """Blocking disk write — must run in thread pool."""
    with open(file_path, "wb") as f:
        f.write(content)


def _decode_and_downscale_sync(file_path: str) -> np.ndarray:
    """Blocking PIL decode + optional cv2 downscale — must run in thread pool."""
...
async def upload_skin_image(
    *,
    ...
    upload_t0 = time.time()
    logger.info(
        f"[{req_id}] SCAN UPLOAD [{unique_filename}]: RECEIVED user_id={current_user.id} "
        f"filename={file.filename!r} type={file.content_type!r}"
    )
    ...
    try:
        ...
        logger.info(f"[{req_id}] [{unique_filename}] Phase1 READ_BYTES: {len(content)} bytes in {(time.time() - phase_t0)*1000:.0f}ms")
    except asyncio.CancelledError:
        logger.warning(f"[{req_id}] [{unique_filename}] Phase1 CANCELLED at {(time.time() - upload_t0)*1000:.0f}ms")
        raise
    except Exception as e:
        logger.error(f"[{req_id}] [{unique_filename}] Phase1 FAILED: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to read uploaded image.")

    if len(content) > MAX_UPLOAD_SIZE:
        logger.warning(f"[{req_id}] [{unique_filename}] REJECTED oversized: {len(content)} bytes")
        raise HTTPException(status_code=413, detail=f"File too large. Maximum {MAX_UPLOAD_SIZE // (1024*1024)} MB allowed.")

    # ── Phase 2: Write to disk (thread pool) ─────────────────
    phase_t0 = time.time()
    try:
        await asyncio.to_thread(_save_file_sync, content, file_path)
        logger.info(f"[{req_id}] [{unique_filename}] Phase2 DISK_WRITE: done in {(time.time() - phase_t0)*1000:.0f}ms")
    except Exception as e:
        logger.error(f"[{req_id}] [{unique_filename}] Phase2 DISK_WRITE FAILED at {(time.time() - upload_t0)*1000:.0f}ms: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to save uploaded image.")

    # ── Phase 3: Decode + downscale (thread pool) ────────────
    phase_t0 = time.time()
    try:
        img = await asyncio.to_thread(_decode_and_downscale_sync, file_path)
        h, w = img.shape[:2]
        logger.info(f"[{req_id}] [{unique_filename}] Phase3 DECODE+DOWNSAMPLE: shape={img.shape} dtype={img.dtype} in {(time.time() - phase_t0)*1000:.0f}ms")
    except Exception as e:
        logger.error(f"[{req_id}] [{unique_filename}] Phase3 DECODE FAILED at {(time.time() - upload_t0)*1000:.0f}ms: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Image analysis failed. Please try a different image.")

    # ── Phase 4: CV analysis (thread pool with timeout) ──────
    t0 = time.time()
    logger.info(f"[{req_id}] [{unique_filename}] Phase4 ANALYSIS: starting pipeline on {w}x{h}")
    try:
        analysis = await asyncio.wait_for(
            asyncio.to_thread(_run_image_analysis_with_logging, img, unique_filename),
            timeout=ANALYSIS_TIMEOUT,
        )
        logger.info(f"[{req_id}] [{unique_filename}] Phase4 ANALYSIS: done in {(time.time() - t0)*1000:.0f}ms class={analysis['classification']} conf={analysis['confidence']}")
    except asyncio.TimeoutError:
        logger.error(f"[{req_id}] [{unique_filename}] Phase4 TIMEOUT after {(time.time() - t0)*1000:.0f}ms (limit={ANALYSIS_TIMEOUT}s)")
        raise HTTPException(
            status_code=500,
            detail="Image analysis took too long. Please try a smaller or less complex image.",
        )
    except asyncio.CancelledError:
        logger.warning(f"[{req_id}] [{unique_filename}] Phase4 CANCELLED at {(time.time() - t0)*1000:.0f}ms")
        raise HTTPException(status_code=499, detail="Request cancelled.")
    except Exception as e:
        logger.error(f"[{req_id}] [{unique_filename}] Phase4 ERROR after {(time.time() - t0)*1000:.0f}ms: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Image analysis failed. Please try a different image.")
    ...
    try:
        ...
        db.commit()
        db.refresh(db_obj)
        logger.info(f"[{req_id}] [{unique_filename}] Phase5 DB_SAVE: done in {(time.time() - phase_t0)*1000:.0f}ms scan_id={db_obj.id}")
    except Exception as e:
        logger.error(f"[{req_id}] [{unique_filename}] Phase5 DB_SAVE FAILED in {(time.time() - phase_t0)*1000:.0f}ms: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Analysis completed but failed to save results. Please try again.")

    total_elapsed = (time.time() - upload_t0) * 1000
    logger.info(f"[{req_id}] [{unique_filename}] DONE: total={total_elapsed:.0f}ms class={db_obj.classification} conf={db_obj.confidence}")
    return db_obj
